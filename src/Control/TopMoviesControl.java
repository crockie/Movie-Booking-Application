...
/**
 * This class controls the display of the top 5 movies by overall ticket sales or reviewer's rating
 */
public class TopMoviesControl implements MainControl{
		...
		while (true) {
			...
			switch (choice) {
                case 1:
					...
					ListView.displayList("Top 5 Movies By Ticket Sales", movieStrings, "No available data");
					break;
                case 2:
					movieStrings = getTopMoviesByOverallRating();
					ListView.displayList("Top 5 Movies By Overall Rating", movieStrings, "No available data");
					break;
				case 3:
	...
	/**
	 * This method determines the top 5 movies by ticket sales and stores it as a list of strings.
	...
	private List<String> getTopMoviesByTicketSales() {
		...
		for (int i = 1; i <= 5 && i <= movieList2.size(); i++) {
			...
			movieTitles.add(i + ". " + movie.getTitle() + " (Total Sales: $" + String.format("%.2f", movie.getTotalSales()) + ")");
		}
		return movieTitles;
	}
	
	/**
	 * This method determines the top 5 movies by overall reviewer's rating and stores it in the form of a list of strings.
	...
	private List<String> getTopMoviesByOverallRating() {
		...
		for (int i = 1; i <= 5 && i < moviesWithRatings.size(); i++) {
			...
			movieTitles.add(i + ". " + movie.getTitle() + " (Average Ratings: " + String.format("%.2f", movie.getAverageRating()) + ")");
		}
		return movieTitles;
	}
}
